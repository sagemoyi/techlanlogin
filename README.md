...
> [!IMPORTANT]
> **首次运行必须在已联网的环境下执行！** Selenium 需要联网下载匹配你 Edge 版本的浏览器驱动并缓存到本地。之后的每次运行（包括断网时开机自动登录）都会使用这个缓存，不再需要联网下载。
>
> 简单来说：**先手动登录一次校园网 → 运行一次脚本 → 以后就全自动了。**
...
首次运行会看到 `✅ 已联网，无需登录，退出`，这是正常的 — 驱动已经缓存好了，下次开机就能自动登录。
